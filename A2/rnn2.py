...
if __name__ == "__main__":
    ...
    model = RNN(50, args.hidden_dim).to('cuda')  # Fill in parameters
    ...
    while not stopping_condition:
        ...
        for minibatch_index in tqdm(range(N // minibatch_size)):
            ...
            for example_index in range(minibatch_size):
                ...
                vectors = torch.tensor(vectors).view(len(vectors), 1, -1).to('cuda')
                output = model(vectors)

                # Get loss
                example_loss = model.compute_Loss(output.view(1,-1), torch.tensor([gold_label]).to('cuda'))
